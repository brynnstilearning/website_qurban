<?php
session_start();
// Pastikan hanya user dengan level 'panitia' atau 'admin' yang bisa mengakses
if (!isset($_SESSION['username']) || ($_SESSION['level'] !== 'panitia' && $_SESSION['level'] !== 'admin')) {
    header("Location: login.php");
    exit();
}

// Tambahkan ini untuk agar tidak error ketika session tidak ada
include "koneksi.php";

// --- PHP BARU: Untuk mengambil data QR & Pengaturan (seperti di wargadas.php) ---
$username_display = htmlspecialchars($_SESSION['username'] ?? 'Panitia');
$qr_token = "Tidak ada QR Code yang terkait dengan akun ini.";
$is_used = 0;

// Cek apakah panitia ini juga terdaftar sebagai peserta (punya id_peserta)
if (isset($_SESSION['id_peserta'])) {
    $id_peserta = $koneksi->real_escape_string($_SESSION['id_peserta']);
    $query_qr = "SELECT qr_token, is_used FROM data WHERE id_peserta = '$id_peserta'";
    $result_qr = $koneksi->query($query_qr);
    
    if ($result_qr && $result_qr->num_rows > 0) {
        $row_qr = $result_qr->fetch_assoc();
        $qr_token = $row_qr['qr_token'];
        $is_used = $row_qr['is_used'];
    } else {
        $qr_token = "Belum ada QR Code untuk Anda sebagai peserta.";
    }
}

// Ambil data pengaturan qurban
...
<head>
    ...
    <title>Dashboard Panitia - QurbanKuy</title>
    ...
    <style>
        ...
        .dashboard-card .qr-img { border: 5px solid #9fcf3f; border-radius: 0; box-shadow: 0 5px 15px rgba(0,0,0,0.1); margin-bottom: 1.5rem; }
        .dashboard-card .qr-token-text { font-family: 'Courier New', Courier, monospace; background-color: #343a40; color: #9fcf3f; padding: 8px 15px; border-radius: 8px; font-weight: 600; display: inline-block; word-break: break-all; }
...
<div class="content-wrapper">
    <div class="dashboard-card">
        ...
        <?php else: ?>
            <div class="alert alert-dark mt-4"><?php echo htmlspecialchars($qr_token); ?></div>
        <?php endif; ?>
        ...
        <div class="info-qurban-box">
            ...
            <a href="data_pengaturan_qurban.php" class="btn btn-primary">
                <i class="bi bi-gear-fill"></i> Kelola Informasi Pengambilan
            </a>
        </div>
    </div>
